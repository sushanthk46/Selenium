...
String path=".\\testdata\\HSIMData.xlsx";//taking xl file from testData
	...
	@DataProvider(name="HOAACTDData")
	public String[][] HOAACTDMenu() throws IOException {
		
String path=".\\testdata\\HOAACTDData.xlsx";//taking xl file from testData
		...
		String logindata[][]=new String[totalrows][totalcols];//created for two dimension array which can store the data user and password
		System.out.println("Columns HOAACTD" +totalrows);
		for(int i=1;i<=totalrows;i++)  //1   //read the data from xl storing in two deminsional array
		{		
			for(int j=0;j<totalcols;j++)  //0    i is rows j is col
			{
